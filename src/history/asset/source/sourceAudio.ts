export const sourceAudio = [
  {
    id: 'mini2022',
    status: true,
    directory: 'source_mini2022/',
    data: [
      {available: true, data: 0, path: 'dragonquest.mp3'},
      {available: true, data: 1, path: 'dearmrjones.mp3'},
      {available: true, data: 2, path: 'tsubame.mp3'},
      {available: true, data: 3, path: 'hitsong90.mp3'},
      {available: true, data: 4, path: 'deeppurple.mp3'},
      {available: true, data: 5, path: 'aldebaran.mp3'},
      {available: true, data: 6, path: 'rupin.mp3'},
      {available: true, data: 7, path: 'disney.mp3'},
      {available: true, data: 8, path: 'shiroi.mp3'},
      {available: true, data: 9, path: 'matsuken.mp3'},
    ],
  }, {
    id: 'main035',
    status: true,
    directory: 'source_2022winds/',
    data: [
      {available: true, data: 0, path: 'thunderbirds.mp3'},
      {available: true, data: 1, path: 'songofsea.mp3'},
      {available: true, data: 2, path: 'sentochihiro.mp3'},
      {available: true, data: 3, path: 'musicforafestival1.mp3', addtitle: '第1楽章 con brio'},
      {available: true, data: 3, path: 'musicforafestival2.mp3', addtitle: '第2楽章 andantino'},
      {available: true, data: 3, path: 'musicforafestival3.mp3', addtitle: '第3楽章 vivo'},
      {available: true, data: 4, path: 'cortege.mp3'},
      {available: true, data: 5, path: 'fontanediroma.mp3'},
      {available: true, data: 6, path: 'yellmarch.mp3'},
      {available: true, data: 8, path: 'blockm.mp3'},
    ],
  }
]
